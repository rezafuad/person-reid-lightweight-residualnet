...
#    ap += ap_tmp
#    #print(i, CMC_tmp[0])
mq = torch.FloatTensor(len(query_label), len(query_feature[0])).zero_()
for i in range(len(query_label)):
    mquery_index1 = np.argwhere(mquery_label==query_label[i])
    mquery_index2 = np.argwhere(mquery_cam==query_cam[i])
    mquery_index =  np.intersect1d(mquery_index1, mquery_index2)
    mq_ = torch.mean(mquery_feature[mquery_index,:], dim=0)
    mq[i, :] = mq_
####
q_g_dist = np.dot(mq, np.transpose(gallery_feature))
...
CMC = CMC.float()
CMC = CMC/len(query_label) #average CMC
print('Multi - top1:%f top5:%f top10:%f mAP:%f'%(CMC[0],CMC[4],CMC[9],ap/len(query_label)))
